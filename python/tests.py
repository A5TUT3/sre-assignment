...
class TestAppHandler(unittest.TestCase):
    ...
    ## (This will validate against K8s deployments health) ##
    def test_deployments_ok(self):
        resp = requests.get(self._get_url("deployments"))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.text, "ok")

    ## (This will validate against K8s deployments health) ##
    def test_pods_ok(self):
        resp = requests.get(self._get_url("pods"))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.text, "ok")
